...
| Sv39 | WIP: data/fetch 3-level 4KiB identity mapping, L1/L2 superpage, load/instruction page fault, SUM, MXR, A/D fault pass |
...
| `make test-os2-min-sv39` | `core/test/os2_min/kernel.c`, `tests.c`, `trap.c` | Pass | S-modeで3段page tableを作成し、`satp.MODE=8`でdata-sideとfetch-sideのSv39を有効化。identity load/store/fetch、2MiB L1 / 1GiB L2 superpage、未map load page fault、SUM=0/1、MXR=0/1、A=0 load fault、D=0 store fault、X=0 instruction page faultを確認 |
