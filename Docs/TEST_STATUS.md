...
| `make test-os2-min OS2_MIN_DEFS=-DOS2_MIN_PMP OS2_MIN_NAME=kernel_pmp CYCLES=260000` | `core/test/os2_min/kernel.c`, `tests.c`, `trap.c`, `firmware.c`, `sbi.c` | Pass | PMP entry1のTOR禁止領域へS-modeからload/store/fetchし、loadは`scause=5`、storeは`scause=7`、fetchは`scause=1`、いずれも`stval=fault address` でS-mode trapへ入ること、禁止storeで保護wordが変化しないことを確認。fetchは`X=1/R=0/W=0`で成功、`R=1/W=1/X=0`でfaultすることも確認 |
