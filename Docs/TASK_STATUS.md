...
| Linux platform | WIP / 128MiB RAM + UART TX + regs + DTB bootargs minimal | `make test-uart`, `make test-uart-regs`, `make dtb`, `make test-linux-bootargs` | OpenSBI/Linux payload target、earlycon、RX/interrupt/PLIC |
